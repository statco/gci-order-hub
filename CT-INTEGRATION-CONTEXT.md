...
**Status:** Client + ledger + routing all merged to `main`. `CT_AUTO_PO_ENABLED` was flipped to **`true` in Vercel production 2026-08-02** — routing now genuinely engages on real orders (`claimOrder()`, `classifyLineItems()`, real ledger rows). `CT_DRY_RUN` is still unset (default `true`), so **no real transmission to CT can happen**. **All four gaps found in the 2026-08-26/28 sessions are now CLOSED and deployed:** § 10 (CT creds in the right Vercel project + `CT_ENVIRONMENT=production` — auth to CT's real production server confirmed working), § 12 (Cowork/`po-drafted` guard, PR #75), § 13/§ 15 (canary override mechanism + dry-run false-alarm fix, PRs #76/#77), § 14 (`order-router.ts`'s stale `TIRE-` prefix removed — direct Shopify orders now actually alert on real orders for the first time, PR #79). **Outstanding, not a code gap:** CT staff have been asked (2026-08-28) not to process PO `GCI-2026-447300` or orders `#1003`/`#1011` while testing continues — lift that once confident it's no longer needed. **Next real blocker to resolve before `CT_DRY_RUN=false`:** CT sandbox/production order-submission behavior is still fundamentally unverified beyond one ambiguous dry-run rehearsal (`#1011`) — no order has ever gone through a genuine successful `submitted` outcome end to end. That's the next milestone, not any known bug.
